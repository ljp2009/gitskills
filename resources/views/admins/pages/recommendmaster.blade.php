        ...
        <div class="am-u-sm-3 ">
            <div class="yma_perview" id="rec_{{$i}}" title="" itemId="0" intro="" img="" type="" >
                <div class="master_box">
                    <img  class="master_avatar" src="http://img.umeiii.com/default.jpg@128w_150h_1e_1c" />
                </div>
                <span class="master_text">未选择</span>
...
<script type="text/javascript" charset="utf-8">
    ...
    function setItemValue(index, title,  img, itemId){
        ...
        $perview.find('img.master_avatar').attr('src',getImgPath(img));
        $perview.attr('img', img);
        $perview.find('span.master_text').text(title);
    ...
    function save(){
        ...
        $.post('/admin/rc/master',data,function(res){
            if(res.res){
                window.location = '/admin/rc/master';
            }
        }).error(function(e){
            alert($(e.responseText).text());
        });
...
@if(isset($masters))
    ...
    setItemValue('{{$masters[$i]->order}}', '{{$masters[$i]->name}}','{{$masters[$i]->img}}','{{$masters[$i]->user_id}}');
    @endfor
@endif
</script>
@stop
