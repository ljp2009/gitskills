...
class RedirectCode{
    ...
    private $settings = ['code' => '', 'mode' => '', 'type' => '', 'resource' => '', 'resource_id' => 0, 'token' => ''];
    private $token = '';
    ...
    public static function getRegistStatus($state){
        return substr($state, 0, 1);
    }
    public static function getType($state){
        $code = substr($state, 1, 4);
        return $code; 
    }
    public static function getResource($state){
        $code = substr($state, 5, 4);
        return $code; 
    }
    public static function getResourceId($state){
        $code = substr($state, 9);
        return intval($code); 
    }
}
